...
This is the single public APK injection entry point. The low-level APK/DEX
machinery lives in ``_inject_apk_core.py``; this front-end configures that
engine for full-SMM injection, validates the complete ``com.spd.mod`` payload,
and patches the target ``WndGame`` menu entry rather than granting a startup
ModAnkh.
"""
...
def find_wndgame_instead_of_dungeon(root: Path, descriptor: str):
    # The core injector asks for Dungeon only so it can patch the legacy startup
    # hook. Full-SMM injection overlays WndGame instead. Other class lookups
    # (notably donor ModAnkh) remain untouched.
