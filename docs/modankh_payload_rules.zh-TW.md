...
- `com.spd.mod.mechanics.ModLootBuff`，以及其 `ModLootStorage`／`ModLoot`／`ModLootBuffOverlay`／`WndModLoot` 與小型 `ModItemKind`／`ModItemOrder`／`ModReusable` helper，供 Debug Console `affect` 使用
